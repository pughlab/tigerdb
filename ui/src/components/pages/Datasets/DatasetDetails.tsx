...
export default function DatasetDetails() {
  ...
  const { data, loading, error, refetch } = useQuery(gql`
		query DatasetDetails($datasetID: ID!) {
      datasets(where: {datasetID: $datasetID}) {
        datasetID
        name
        # description
        # files {
  ...
  const [{ name, description}]: [{ datasetID: String, name: String, description: String }] = data.datasets
      ...
      <Grid>
        ...
        <Grid.Row divided>
          <Grid.Column width={10}>
            ...
            {/* upload happens here: */}
            <MinioBucket datasetID={`${datasetID}`} />
          </Grid.Column>
          {/* <Grid.Column width={6}>
